def display_user_selected_points(st):
    ...
    for i, p in enumerate(st.session_state.selected_points):
        ...
        with col2:
            if st.button("Remove", key=f"remove_{i}"):  # remove points
                removed_id = p["id"]
    ...
    if st.session_state.source_id not in source_ids:
        st.session_state.source_id = source_ids[0] # update source id
    ...
    st.session_state.source_id = source_options[selected_source_label]  # update source id
